...
def process_song_data(spark, input_data, output_data):
    """
    This function loads songs data from S3 then process the songs/artist tables and package them back to S3.
...
def process_log_data(spark, input_data, output_data):
    """
    This function loads log data from S3 then processes the songs/artist tables and package them back to S3.
    :param spark: spark session
    :param input_data: input data path
    :param output_data: output data path
    """
    ...
    songplays_table = df.join(song_df, song_df.artist_name == df.artist, "inner").distinct().select(col("start_time"), col("userId"), col("level"), col("sessionId"), \
                col("location"), col("userAgent"), col("song_id"), col("artist_id")).withColumn("songplay_id", monotonically_increasing_id())
